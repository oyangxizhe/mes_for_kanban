...
{
    public partial class BILL_NATURE : Form
    {
...
BILL_NATURE A ";
        ...
        private void dgvStateControl()
        {
            ...
            for (i = 0; i < numCols1; i++)
            {
                ...
                dataGridView1.Columns["单据性质"].Width = 120;
        ...
        private void save()
        {
            ...
            if (!bc.exists("SELECT BNID FROM BILL_NATURE WHERE BNID='" + textBox1 .Text + "'"))
            {

                if (bc.exists("SELECT * FROM BILL_NATURE WHERE BILL_NATURE='"+textBox2 .Text +"'"))
                {
                    hint.Text = "此单据性质已经存在";
                ...
                {
                    basec.getcoms(@"INSERT INTO BILL_NATURE(BNID,BILL_NATURE,MAKERID,DATE,YEAR,
            ...
            else
            {
                ...
                {
                    basec.getcoms(@"UPDATE BILL_NATURE SET BILL_NATURE='" + textBox2.Text + "',MAKERID='" + varMakerID +
                          "',DATE='" + varDate + "' WHERE BNID='" + textBox1.Text + "'");
                    IFExecution_SUCCESS = true;
                    bind();
                }
            }
        ...
        {
            ClearText();
            textBox1.Text = cBILL_NATURE.GETID();
            textBox2.Focus();
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dt = bc.getdt(sql+" WHERE A.BNID LIKE '%"+textBox4.Text +"%' AND A.BILL_NATURE LIKE '%"+textBox5 .Text +"%'");
                if (dt.Rows.Count > 0)
                {
                    dataGridView1.DataSource = dt;
                    dgvStateControl();
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            {
                IFExecution_SUCCESS = false;
                string strSql = "DELETE FROM BILL_NATURE WHERE BNID='" + id + "'";
