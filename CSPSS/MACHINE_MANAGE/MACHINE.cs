...
namespace CSPSS.MACHINE_MANAGE
{
    public partial class MACHINE : Form
    {
        ...
        private void bind()
        {
            ...
            if (MRID != null)
            {
                MessageBox.Show(MRID);
                dt = basec.getdts(string.Format(cmachine_and_group.sqlfi + " WHERE A.MRID='{0}'", MRID));
            }
            else
            {
                dt = basec.getdts(cMACHINE.sql);
            }
            ...
            if (bc.GET_IFExecutionSUCCESS_HINT_INFO(cMACHINE.IFExecution_SUCCESS) != "")
            {
                hint.Text = bc.GET_IFExecutionSUCCESS_HINT_INFO(cMACHINE.IFExecution_SUCCESS);
            }
            else
            {
                hint.Text = "";
        ...
        private void save()
        {
            ...
            cMACHINE.EMID = LOGIN.EMID;
            cMACHINE.MAID = IDO;
            cMACHINE .MACHINE_ID =textBox2.Text ;
            cMACHINE .MACHINE =textBox3.Text ;
            cMACHINE.save();
            IFExecution_SUCCESS = cMACHINE.IFExecution_SUCCESS;
            hint.Text = cMACHINE.ErrowInfo;
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dt = bc.getdt(cMACHINE.sql + " WHERE A.MACHINE_ID LIKE '%" + textBox4.Text + "%' AND A.MACHINE LIKE '%" + textBox5.Text + "%'");
                if (dt.Rows.Count > 0)
                {
                    dataGridView1.DataSource = dt;
                    dgvStateControl();
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            if (bc.exists(string.Format("SELECT * FROM POSTING_MST WHERE MAID='{0}'", bc.RETURN_MAID(id))))
            {
                hint.Text = string.Format("机台 {0} 已经存在过账记录不允许删除", id);
            ...
            else
            {
                ...
                string strSql = "DELETE FROM MACHINE WHERE MACHINE_ID='" + id + "'";
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (v1 != "")
            {
                IDO = bc.getOnlyString("SELECT MAID FROM MACHINE WHERE MACHINE_ID='" + Convert.ToString(dataGridView1[1, dataGridView1.CurrentCell.RowIndex].Value).Trim() + "'");
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.Enabled == true)
            {
                ...
                if (select == 0)
                {
                    PRODUCTION_MANAGE.POSTINGT.MAID = v1;
                    PRODUCTION_MANAGE.POSTINGT.MACHINE_ID = v2;
                    PRODUCTION_MANAGE.POSTINGT.MACHINE = v3;
