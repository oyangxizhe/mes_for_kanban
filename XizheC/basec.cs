...
WHERE A.WAREID='" + wareid + "' AND A.COID='" + coid + "' GROUP BY A.WAREID,A.COID,A.SIID,B.COLOR,C.SIZE HAVING SUM(A.GECOUNT)-SUM(A.MRCOUNT)>0 ORDER BY A.WAREID,A.COID,A.SIID";
        ...
        {
            bool b = false;
            if (this.exists("SELECT * FROM WAREINFO WHERE MAKERID='" + EMID + "'"))
            {
                b = true;
                ErrowInfo = "该工号已经在品号信息中使用了，不允许删除！";
            }
           else  if (this.exists("SELECT * FROM USERINFO WHERE MAKERID='" + EMID + "' OR EMID='" + EMID + "'"))
            {
                b = true;
            ...
            else if (this.exists("SELECT * FROM STEP WHERE MAKERID='" + EMID + "'"))
            {
                b = true;
                ErrowInfo = "该工号已经在站别作业中使用了，不允许删除！";
            }
            else if (this.exists("SELECT * FROM FLOW_CHART_MST WHERE MAKERID='" + EMID + "'"))
            {
                b = true;
                ErrowInfo = "该工号已经在途程图作业中使用了，不允许删除！";
            }
            else if (this.exists("SELECT * FROM WORKORDER_MST WHERE MAKERID='" + EMID + "'"))
            {
                b = true;
                ErrowInfo = "该工号已经在工单作业中使用了，不允许删除！";
